...
# Optional dependency: rapidfuzz (fast). If missing, fallback to difflib.
try:
...
# ✅ UPDATED: Mortgage amount extractor (uses "DATE OF NOTICE")
def _extract_mortgage_amount_from_date_of_notice(text: str) -> int:
    """
    Looks for a dollar amount near the phrase 'DATE OF NOTICE'.
    Common patterns:
    ...
    """
    # Use DOTALL to allow separators like dots/whitespace/newlines between phrase and amount
    patterns = [
        r"DATE\s+OF\s+NOTICE\s*[:\-]?\s*\$?\s*([0-9][0-9,]*\.?\d{0,2})",
        r"DATE\s+OF\s+NOTICE.{0,80}?\$?\s*([0-9][0-9,]*\.?\d{0,2})",
    ]
    for pat in patterns:
    ...
    *,
    ...
    for i, url in enumerate(ad_links, start=1):
        try:
            ...
            # ✅ UPDATED mortgage amount: use "DATE OF NOTICE"
            mortgage_amount = _extract_mortgage_amount_from_date_of_notice(text)
            ...
            out.append(
                {
                    ...
                    "mortgage_amount": int(mortgage_amount or 0),
                    "signal_strength": 35,
                    "signal_details": (
                        f"Foreclosure notice; sale_date={sale_date or 'unknown'}; "
                        f"county={county or 'unknown'}; mortgage_amount=${int(mortgage_amount or 0):,}"
                        if mortgage_amount
...
def merge_notices_into_leads(
    ...
    *,
    ...
    Also merges foreclosure mortgage_amount into:
    ...
    # -------------------------
    # Probate match (simple heuristic by owner last name)
    # -------------------------
