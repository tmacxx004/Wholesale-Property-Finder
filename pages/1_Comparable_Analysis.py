...
with colB:
    st.link_button("⬅️ Back to Results", "/")
...
delq_year = subj.get("earliest_delq_year", None)
...
flags = []
if forfeit: flags.append("Tax-forfeit indicator")
if years_delq: flags.append(f"Delinquent ~{years_delq} yrs")
if absentee: flags.append("Absentee owner")
if homestead and str(homestead).upper() != "H": flags.append(f"Non-homestead ({homestead})")
...
st.caption("Tip: tighten the value band for more relevant comps, widen it if you’re in an area with low turnover.")

# Add a quick “ARV hint” (simple heuristic)
# NOTE: This is *not* an appraisal; it’s a directional signal.
vals = comps_df["market_value_total"].dropna()
if len(vals) >= 3:
    arv_hint = vals.median()
    st.success(f"Directional ARV hint (median market value of comps): **${arv_hint:,.0f}**")
...
st.markdown(
    """
- Confirm distress + motivation signals (delinquency years, forfeiture, absentee, code violations if you stacked MPLS).
- Pull photos / condition signals (drive-by, street view, seller call intel).
- Set offer range using: **ARV × (0.70–0.80) − Repairs − Assignment fee** (adjust to your buyer pool).
- Then verify title / liens with your preferred process.
"""
)
